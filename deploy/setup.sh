...
# Idempotent GCP bootstrap for the elixir-training slides deployment.
# Re-runnable: each step either creates the resource or no-ops if it already exists.

PROJECT_ID="${PROJECT_ID:-ristkari-dev}"
REGION="${REGION:-europe-north1}"
AR_REPO="${AR_REPO:-elixir-training}"
SERVICE_NAME="${SERVICE_NAME:-elixir-training-slides}"
SA_NAME="${SA_NAME:-github-deploy-elixir}"
...
WIF_PROVIDER="${WIF_PROVIDER:-github-elixir-training}"
GITHUB_REPO="${GITHUB_REPO:-ristkari-dev/elixir-training}"
...
if gcloud artifacts repositories describe "$AR_REPO" \
    ...
    gcloud artifacts repositories create "$AR_REPO" \
        ...
        --location="$REGION" \
        --description="elixir-training container images" \
        --project="$PROJECT_ID"
fi
...
if gcloud iam service-accounts describe "$SA_EMAIL" --project="$PROJECT_ID" >/dev/null 2>&1; then
    ...
    gcloud iam service-accounts create "$SA_NAME" \
        --display-name="GitHub Actions deploy for elixir-training" \
        --project="$PROJECT_ID"
fi
...
echo "      --domain=elixir.ristkari.dev \\"
echo "      --region=$REGION --project=$PROJECT_ID"
